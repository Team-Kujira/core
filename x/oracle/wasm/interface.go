...
func (querier WasmQuerier) QueryCustom(ctx sdk.Context, data json.RawMessage) ([]byte, error) {
	...
	if params.ExchangeRates != nil {
		// LUNA / BASE_DENOM
		baseDenomExchangeRate, err := querier.keeper.GetExchangeRate(ctx, params.ExchangeRates.BaseDenom)
		if err != nil {
			return nil, err
		}
		...
		for _, quoteDenom := range params.ExchangeRates.QuoteDenoms {
			// LUNA / QUOTE_DENOM
			quoteDenomExchangeRate, err := querier.keeper.GetExchangeRate(ctx, quoteDenom)
			if err != nil {
				continue
			}
